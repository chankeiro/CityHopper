...
namespace Bercetech.Games.Fleepas
{
    ...
    public class SinglePlayerManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject _scanInstructions;
        [SerializeField]
        private GameObject _cloudAnchorPrompt;
        [SerializeField]
        private GameObject _cloudAnchorTestScan;
        [SerializeField]
        ...
        [SerializeField]
        protected TextMeshProUGUI _singlePlayerNextRoundText;
        [SerializeField]
        ...
        [SerializeField]
        protected MeshMaterialChanger _meshMaterialChanger;
        [SerializeField]
        private GameObject _finalSound;
        [SerializeField]
        private ARFManager _arfManager;
        [SerializeField]
        private GameObject _hostResolveAnchorScreen;
        [SerializeField]
        ...
        private static SinglePlayerManager _sharedInstance;
        public static SinglePlayerManager SharedInstance => _sharedInstance;
        ...
        protected virtual void Start()
        {
            ...
            PlayerMode.SharedInstance.GameWarming();

            if (PlayerMode.SharedInstance.FleepSiteId == "")
            {
                // In case the user is not on a fleepsite
                // show scanning instructions after one second
                Observable.Timer(TimeSpan.FromSeconds(1f))
                    .Subscribe(_ => _scanInstructions.SetActive(true))
                    .AddTo(disposables);
            }
            else
            {
            ...
            Score.SharedInstance.FinalScoreCount.Subscribe(_ =>
            {
                ...
                RoundReset();
                // Launch the review information request (if conditions are met),
                // since google recommends to do it a few seconds before of actually need it
                GooglePlayReviewManager.SharedInstance.RequestReviewInfo();
        ...
        public void ReturnToNativeApp()
        {
            ...
            disposables.Clear();
            // Destroying Meshes from manager
            ARMeshData.SharedInstance.DestroyMeshesFromManager();
        ...
        {
            if (_fleepSiteCreationStarted)
                _cloudAnchorTestScan.SetActive(true);
            else if (PlayerMode.SharedInstance.CanCreateFleepSite)
            {
                _cloudAnchorPrompt.SetActive(true);
                // Flagging the fleepSite creation process so, in case
                // we need to rescan and come back to this same function
                // we move directly to the scan test instead of asking again
                // to the users if they want to create a fleepSite
                _fleepSiteCreationStarted = true;
            }
            else
                _singlePlayerStartGame.SetActive(true);
        }

        protected void EndSinglePlayerRound()
        {
            ...
            if (PlayerMode.SharedInstance.PlayerUId != "null" // Null with string, since this is a string parameter that comes from Main Activity
                && PlayerMode.SharedInstance.FleepSiteId != "") 
            {
                // Check if the user has got some new prize based on points
                foreach (FleepSitePrize prize in PlayerMode.SharedInstance.FleepSitePrizes)
                {
                    ...
                    if (_totalScore >= prize.MinPoints && prize.MinPosition == 0)
                    {
                        ...
                        if (prize.MaxNumber == 0)
                        {
                            ...
                            if (Mathf.Max(PlayerMode.SharedInstance.UserFleepSiteTopScore , _scoreAtStart) < prize.MinPoints) 
                            {
                        ...
                        else
                        {
                            ...
                            if (!PlayerMode.SharedInstance.UserFleepSitePrizesGranted.Contains(prize.PrizeId)
        ...
        public void ShowPrizeScreen()
        {
            if (_newPrizeScreens.Count > _newPrizeScreenIndex)
            {
                ...
                string prizeDescription = PlayerMode.SharedInstance.FleepSitePrizes
        ...
        public void ReceiveFleepSitePrizeLimitReachedFromAndroid(string prizeLimitResponseParams)
        {
            ...
            } else
            {
                ...
                // in the prizes list
                PlayerMode.SharedInstance.FinishFleepSitePrize(prizeId);
            }
        ...
        {
            // Enabling persistent anchor mode
            CloudAnchorsModeHolder.SharedInstance.SetResolvingMode();
            _hostResolveAnchorScreen.SetActive(true);
        }

        private void GenerateStringVar()
        {
            // We subscribe only once onStart
            _nextRoundString.StringChanged += OnStringChanged; 
            _matchRoundStringVar = _nextRoundString["matchRound"] as IntVariable;
            _roundScoreStringVar = _nextRoundString["roundScore"] as IntVariable;
            _nextRoundScoreThresholdStringVar = _nextRoundString["nextRoundScoreThreshold"] as IntVariable;
            _totalScoreStringVar = _nextRoundString["totalScore"] as IntVariable;
        }

        private void OnStringChanged(string str)
        {
            _singlePlayerNextRoundText.text = str;
        }

        private void OnDestroy()
        {
            // Need to unsubscribe everytime we destroy the scene
            _nextRoundString.StringChanged -= OnStringChanged;
        }
