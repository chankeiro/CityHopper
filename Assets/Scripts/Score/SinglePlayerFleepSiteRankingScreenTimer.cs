...
{
    public class SinglePlayerFleepSiteRankingScreenTimer : MonoBehaviour
    {
        [SerializeField]
        ...
        private void Awake()
        {
            ...
            //{
                SetFleepSiteRankingCurrentGame(5);
            //}).AddTo(gameObject);
        ...
        void OnEnable()
        {
            ...
            _currentScoreText.text = "Your Time: " + TimeSpan.FromMilliseconds(Clock.SharedInstance.TimePassed).ToString(@"m\mss\s");
            _bestScoreText.text = "Best Time: 1m03s";
            ...
            gameObject.GetComponent<RectTransform>().SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, Mathf.Min(800f + PlayerModeGeospatial.SharedInstance.FleepSitePrizes.Count*100f, 1500));

            // In case of FleepSite, showing current ranking and list of prizes in case there are
            if (PlayerModeGeospatial.SharedInstance.FleepSitePrizes.Count > 0)
            {
                ...
                foreach (var prize in PlayerModeGeospatial.SharedInstance.FleepSitePrizes)
                {
                    //// WON PRIZES
